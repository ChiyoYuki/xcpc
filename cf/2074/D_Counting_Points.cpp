...
{
    i8 ans=sqrt(n);
    if(ans*ans<=n&&(ans+1)*(ans+1)>n) return ans;
    ans++;
    if(ans*ans<=n&&(ans+1)*(ans+1)>n) return ans;
    ans++;
    if(ans*ans<=n&&(ans+1)*(ans+1)>n) return ans;
...
void solve(void)
{
    ...
    cin>>n>>m;
    vpii a(n);
    for(i=0;i<n;i++) cin>>a[i].st;
    for(i=0;i<n;i++) cin>>a[i].nd;
    sort(a.begin(),a.end());
    mii b;

    for(i=0;i<n;i++)
    {
        for(j=a[i].st-a[i].nd;j<=a[i].st+a[i].nd;j++)
        {
            i8 fuck=(a[i].nd)*(a[i].nd)-(j-a[i].st)*(j-a[i].st);
            fuck=1+2*SQRT(fuck);
            auto xxx=b.find(j);
            if(xxx==b.end()) b[j]=fuck;
            else b[j]=max(b[j],fuck);
        }
    }
