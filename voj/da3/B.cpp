    ...
    for (i = 1; i <= n; i++)
    {
        for (j = 1; j <= m; j++)
        {
            if (a[i][j] == '#')
            {
                ...
                while(!bfs.empty())
                {
                    if(a[bfs.front().st-1][bfs.front().nd]=='#') bfs.push_back(make_pair(bfs.front().st-1,bfs.front().nd));
                    if(a[bfs.front().st+1][bfs.front().nd]=='#') bfs.push_back(make_pair(bfs.front().st+1,bfs.front().nd));
                    if(a[bfs.front().st][bfs.front().nd-1]=='#') bfs.push_back(make_pair(bfs.front().st,bfs.front().nd-1));
                    if(a[bfs.front().st][bfs.front().nd+1]=='#') bfs.push_back(make_pair(bfs.front().st,bfs.front().nd+1));
                    a[bfs.front().st][bfs.front().nd]='.';
                    bfs.pop_front();
                }
            }
        }
